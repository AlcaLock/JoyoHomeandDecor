...
export class grupoComponenteController {
  prisma = new PrismaClient();

  get = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const gruposComponente = await this.prisma.grupoComponente.findMany({
        orderBy: { nombre: 'asc' }
      });
      res.json(gruposComponente);
    } catch (error) {
      next(error);
    }
  };
  ...
  getById = async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const etiqueta = await this.prisma.grupoComponente.findUnique({
        where: { id }
      });
